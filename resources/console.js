function Console(gba) {
	...
	this.tileView = new TileViewer(
		gba.video.renderPath.vram,
		gba.video.renderPath.palette
	);
	this.update();
	...
	if (doScroll) {
		...
		var scrollUp = function () {
			if (ul.scrollTop == last) {
				ul.scrollTop =
					(ul.scrollHeight - ul.offsetHeight) * 0.2 + last * 0.8;
				last = ul.scrollTop;
		...
		if (self.stillRunning) {
			try {
				...
				if (
					...
					self.breakpoints[self.cpu.gprs[self.cpu.PC]]
				) {
					self.breakpointHit();
					return;
				}
...
Console.prototype.breakpointHit = function () {
	...
	this.log(
		this.gba.LOG_DEBUG,
		'Hit breakpoint at ' + hex(this.cpu.gprs[this.cpu.PC])
	);
};
...
Console.prototype.testBreakpoints = function () {
	if (
		...
		this.breakpoints[this.cpu.gprs[this.cpu.PC]]
	) {
		this.breakpointHit();
		return false;
	}
...
Memory = function (mmu) {
	...
	var self = this;
	this.ul.parentElement.addEventListener(
		'scroll',
		function (e) {
			self.scroll(e);
		},
